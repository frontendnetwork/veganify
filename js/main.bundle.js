! function(e, t) { "use strict"; "object" == typeof module && "object" == typeof module.exports ? module.exports = e.document ? t(e, !0) : function(e) { if (!e.document) throw new Error("jQuery requires a window with a document"); return t(e) } : t(e) }("undefined" != typeof window ? window : this, function(C, e) {
    ...
    var d = function(n) {
        ...
        return me.prototype = b.filters = b.pseudos, b.setFilters = new me, h = se.tokenize = function(e, t) { var n, r, i, o, a, s, u, l = x[e + " "]; if (l) return t ? 0 : l.slice(0);
            ...
            return a
        }, g = se.select = function(e, t, n, r) { var i, o, a, s, u, l = "function" == typeof e && e,
    ...
    }), ct = { set: function(e, t, n) { return !1 === t ? S.removeAttr(e, n) : e.setAttribute(n, n), n } }, S.each(S.expr.match.bool.source.match(/\w+/g), function(e, t) { var a = ft[t] || S.find.attr;
    ...
    function Ft(e, t) { var n, r, i = S.ajaxSettings.flatOptions || {}; for (n in t) void 0 !== t[n] && ((i[n] ? e : r || (r = {}))[n] = t[n]); return r && S.extend(!0, e, r), e } Mt.href = bt.href, S.extend({
        ...
        ajax: function(e, t) {
            ...
            if (u = "abort", b.add(v.complete), T.done(v.success), T.fail(v.error), c = Wt(Pt, v, t, T)) { if (T.readyState = 1, g && m.trigger("ajaxSend", [T, v]), h) return T;
...
BarcodeScanner = {
    Config: { Multiple: true, DecodeFormats: ["Code128", "Code93", "Code39", "EAN-13", "2Of5", "Inter2Of5", "Codabar"], ForceUnique: true, LocalizationFeedback: false, SkipOrientation: false },
    ...
    init: function() { BarcodeScanner.ScanCanvas = BarcodeScanner.FixCanvas(document.createElement("canvas"));
        BarcodeScanner.ScanCanvas.width = 640;
        BarcodeScanner.ScanCanvas.height = 480;
        BarcodeScanner.ScanContext = BarcodeScanner.ScanCanvas.getContext("2d"); var script = document.createElement('script');
        script.src = "js/exif.js";
        script.type = 'text/javascript';
        document.getElementsByTagName('head').item(0).appendChild(script); },
    SetRotationSkip: function(value) { BarcodeScanner.Config.SkipOrientation = value; },
    SetImageCallback: function(callBack) { BarcodeScanner.ImageCallback = callBack; },
    setStreamCallback: function(callBack) { BarcodeScanner.streamCallback = callBack; },
    SetLocalizationCallback: function(callBack) { BarcodeScanner.LocalizationCallback = callBack;
        BarcodeScanner.Config.LocalizationFeedback = true; },
    SwitchLocalizationFeedback: function(bool) { BarcodeScanner.Config.LocalizationFeedback = bool; },
    DecodeSingleBarcode: function() { BarcodeScanner.Config.Multiple = false; },
    DecodeMultiple: function() { BarcodeScanner.Config.Multiple = true; },
    SetDecodeFormats: function(formats) {
        BarcodeScanner.Config.DecodeFormats = [];
        for (var i = 0; i < formats.length; i++) { if (BarcodeScanner.SupportedFormats.indexOf(formats[i]) != -1) { BarcodeScanner.Config.DecodeFormats.push(formats[i]); } }
        if (BarcodeScanner.Config.DecodeFormats.length == 0) { BarcodeScanner.Config.DecodeFormats = BarcodeScanner.SupportedFormats.slice(); }
    },
    SkipFormats: function(formats) { for (var i = 0; i < formats.length; i++) { var index = BarcodeScanner.Config.DecodeFormats.indexOf(formats[i]); if (index >= 0) { BarcodeScanner.Config.DecodeFormats.splice(index, 1); } } },
    AddFormats: function(formats) { for (var i = 0; i < formats.length; i++) { if (BarcodeScanner.SupportedFormats.indexOf(formats[i]) != -1) { if (BarcodeScanner.Config.DecodeFormats.indexOf(formats[i]) == -1) { BarcodeScanner.Config.DecodeFormats.push(formats[i]); } } } },
    BarcodeScannerImageCallback: function(e) {
        if (e.data.success == "localization") {
            if (BarcodeScanner.Config.LocalizationFeedback) { BarcodeScanner.LocalizationCallback(e.data.result); }
            return;
        }
        if (e.data.success == "orientationData") { BarcodeScanner.OrientationCallback(e.data.result); return; }
        var filteredData = [];
        for (var i = 0; i < e.data.result.length; i++) { if (BarcodeScanner.Decoded.indexOf(e.data.result[i].Value) == -1 || BarcodeScanner.Config.ForceUnique == false) { filteredData.push(e.data.result[i]); if (BarcodeScanner.Config.ForceUnique) BarcodeScanner.Decoded.push(e.data.result[i].Value); } }
    ...
    BarcodeScannerStreamCallback: function(e) {
        ...
        if (e.data.success && BarcodeScanner.DecodeStreamActive) {
            var filteredData = [];
            for (var i = 0; i < e.data.result.length; i++) { if (BarcodeScanner.Decoded.indexOf(e.data.result[i].Value) == -1 || BarcodeScanner.ForceUnique == false) { filteredData.push(e.data.result[i]); if (BarcodeScanner.ForceUnique) BarcodeScanner.Decoded.push(e.data.result[i].Value); } }
            if (filteredData.length > 0) { BarcodeScanner.streamCallback(filteredData); }
        }
        if (BarcodeScanner.DecodeStreamActive) { BarcodeScanner.ScanContext.drawImage(BarcodeScanner.Stream, 0, 0, BarcodeScanner.ScanCanvas.width, BarcodeScanner.ScanCanvas.height);
            BarcodeScanner.DecoderWorker.postMessage({ scan: BarcodeScanner.ScanContext.getImageData(0, 0, BarcodeScanner.ScanCanvas.width, BarcodeScanner.ScanCanvas.height).data, scanWidth: BarcodeScanner.ScanCanvas.width, scanHeight: BarcodeScanner.ScanCanvas.height, multiple: BarcodeScanner.Config.Multiple, decodeFormats: BarcodeScanner.Config.DecodeFormats, cmd: "normal", rotation: 1, }); }
        if (!BarcodeScanner.DecodeStreamActive) { BarcodeScanner.Decoded = []; }
    },
    DecodeImage: function(image) { if (image instanceof Image || image instanceof HTMLImageElement) { image.exifdata = false; if (image.complete) { if (BarcodeScanner.Config.SkipOrientation) { BarcodeScanner.BarcodeScannerDecodeImage(image, 1, ""); } else { EXIF.getData(image, function(exifImage) { var orientation = EXIF.getTag(exifImage, "Orientation"); var sceneType = EXIF.getTag(exifImage, "SceneCaptureType"); if (typeof orientation != 'number') orientation = 1;
                        BarcodeScanner.BarcodeScannerDecodeImage(exifImage, orientation, sceneType); }); } } else { var img = new Image();
                img.onload = function() { if (BarcodeScanner.Config.SkipOrientation) { BarcodeScanner.BarcodeScannerDecodeImage(img, 1, ""); } else { EXIF.getData(this, function(exifImage) { var orientation = EXIF.getTag(exifImage, "Orientation"); var sceneType = EXIF.getTag(exifImage, "SceneCaptureType"); if (typeof orientation != 'number') orientation = 1;
                            BarcodeScanner.BarcodeScannerDecodeImage(exifImage, orientation, sceneType); }); } };
                img.src = image.src; } } else { var img = new Image();
            img.onload = function() { if (BarcodeScanner.Config.SkipOrientation) { BarcodeScanner.BarcodeScannerDecodeImage(img, 1, ""); } else { EXIF.getData(this, function(exifImage) { var orientation = EXIF.getTag(exifImage, "Orientation"); var sceneType = EXIF.getTag(exifImage, "SceneCaptureType"); if (typeof orientation != 'number') orientation = 1;
                        BarcodeScanner.BarcodeScannerDecodeImage(exifImage, orientation, sceneType); }); } };
            img.src = image; } },
    DecodeStream: function(stream) { BarcodeScanner.Stream = stream;
        ...
        BarcodeScanner.DecoderWorker.postMessage({ scan: BarcodeScanner.ScanContext.getImageData(0, 0, BarcodeScanner.ScanCanvas.width, BarcodeScanner.ScanCanvas.height).data, scanWidth: BarcodeScanner.ScanCanvas.width, scanHeight: BarcodeScanner.ScanCanvas.height, multiple: BarcodeScanner.Config.Multiple, decodeFormats: BarcodeScanner.Config.DecodeFormats, cmd: "normal", rotation: 1, }); },
    StopStreamDecode: function() { BarcodeScanner.DecodeStreamActive = false;
        BarcodeScanner.Decoded = []; },
    BarcodeScannerDecodeImage: function(image, orientation, sceneCaptureType) {
        if (orientation == 8 || orientation == 6) { if (sceneCaptureType == "Landscape" && image.width > image.height) { orientation = 1;
                BarcodeScanner.ScanCanvas.width = 640;
                BarcodeScanner.ScanCanvas.height = 480; } else { BarcodeScanner.ScanCanvas.width = 480;
                BarcodeScanner.ScanCanvas.height = 640; } } else { BarcodeScanner.ScanCanvas.width = 640;
            BarcodeScanner.ScanCanvas.height = 480; }
        ...
        BarcodeScanner.DecoderWorker.postMessage({ scan: BarcodeScanner.ScanContext.getImageData(0, 0, BarcodeScanner.ScanCanvas.width, BarcodeScanner.ScanCanvas.height).data, scanWidth: BarcodeScanner.ScanCanvas.width, scanHeight: BarcodeScanner.ScanCanvas.height, multiple: BarcodeScanner.Config.Multiple, decodeFormats: BarcodeScanner.Config.DecodeFormats, cmd: "normal", rotation: orientation, postOrientation: BarcodeScanner.PostOrientation });
    },
    DetectVerticalSquash: function(img) {
        ...
        try { var data = ctx.getImageData(0, 0, 1, ih).data; } catch (err) { console.log("Cannot check verticalSquash: CORS?"); return 1; }
        var sy = 0;
        var ey = ih;
        var py = ih;
        while (py > sy) {
    ...
    FixCanvas: function(canvas) {
        ...
        ctx.drawImage = function(img, sx, sy, sw, sh, dx, dy, dw, dh) {
            ...
            if (!!img && img.nodeName == 'IMG') { vertSquashRatio = BarcodeScanner.DetectVerticalSquash(img);
...
(function(global,factory){typeof exports==="object"&&typeof module!=="undefined"?module.exports=factory():typeof define==="function"&&define.amd?define(factory):(global.pwaInstallPrompt=factory());}(this,(function(){const DEFAULTS={active_class:"is-active",closer:".pwa-install-prompt__overlay",condition:null,container:".pwa-install-prompt__container",expires:180,show_after:90,};const NEEDS_TO_SEE_PROMPT=function(options){ if(options.condition!==null){return options.condition;} 
if(navigator.standalone){return false;} 
const NOW=Date.now();const THEN=document.cookie.match(/pwa_install_prompt_last_visit=([0-9]+)/);document.cookie="pwa_install_prompt_last_visit="+NOW+"; max-age="+(60*60*24*options.expires)+"; SameSite=None; Secure"; const DAYS=THEN&&THEN.length>=2?Math.ceil((parseInt(THEN[1])-NOW)/(1000*60*60*24)):NaN;const IOS=["iPad","iPhone","iPod"].indexOf(navigator.platform)>-1; return(isNaN(DAYS)||DAYS>options.show_after)&&IOS;};const INSTALL_PROMPT=(function(container=DEFAULTS.container,options={}){const PROMPT=this;options.active_class=options.active_class||DEFAULTS.active_class;options.closer=options.closer||DEFAULTS.closer;options.condition=options.condition||DEFAULTS.condition;options.container=container;options.expires=options.expires||DEFAULTS.expires;options.show_after=options.show_after||DEFAULTS.show_after;options.on=options.on||false; PROMPT.container=container;PROMPT.options=options;PROMPT.eventListeners={};if(PROMPT.options.on){ Object.keys(PROMPT.options.on).forEach(function(event_name){PROMPT.on(event_name,PROMPT.options.on[event_name]);});} 
if(NEEDS_TO_SEE_PROMPT(PROMPT.options)){PROMPT.open();}
const CLOSER=document.querySelector(PROMPT.options.closer); if(CLOSER){CLOSER.addEventListener("click",function(){PROMPT.close();});}});INSTALL_PROMPT.prototype.on=function(events,handler){const PROMPT=this; if(typeof handler!=="function"){return PROMPT;} 
events.split(" ").forEach(function(event){if(!PROMPT.eventListeners[event]){PROMPT.eventListeners[event]=[];}
PROMPT.eventListeners[event].push(handler);});return PROMPT;};INSTALL_PROMPT.prototype.emit=function(){const PROMPT=this; if(!PROMPT.eventListeners){return PROMPT;} 
const EVENTS=Array.isArray(arguments[0])?arguments[0]:arguments[0].split(" ");EVENTS.forEach(function(event){ if(PROMPT.eventListeners&&PROMPT.eventListeners[event]){ PROMPT.eventListeners[event].forEach(function(eventHandler){eventHandler.apply(PROMPT);});}});return PROMPT;};INSTALL_PROMPT.prototype.open=function(){const PROMPT=this;const CONTAINER=document.querySelector(PROMPT.options.container);PROMPT.emit("beforeOpen");if(CONTAINER){CONTAINER.classList.add(PROMPT.options.active_class);}
PROMPT.emit("afterOpen");};INSTALL_PROMPT.prototype.close=function(){const PROMPT=this;const CONTAINER=document.querySelector(PROMPT.options.container);PROMPT.emit("beforeClose");if(CONTAINER){CONTAINER.classList.remove(PROMPT.options.active_class);}
PROMPT.emit("afterClose");};return INSTALL_PROMPT;})));
